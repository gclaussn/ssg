...
public class SitePluginGoalTaskEndpoint implements ThreadFactory {

  private static final Logger LOGGER = LoggerFactory.getLogger(SitePluginGoalTaskEndpoint.class);

  private static final String THREAD_NAME = "ssg-exec";

  private final ExecutorService executorService;

  private final Map<String, SitePluginGoalTask> tasks;

  public SitePluginGoalTaskEndpoint() {
  ...
  protected void sendMessages(Session session, Optional<String> taskId) {
    ...
    SitePluginGoalTask task = tasks.get(taskId.get());
    if (task == null) {
      return;
    }
  ...
  public String submit(Site site, String pluginGoal, Map<String, Object> properties) {
    SitePluginGoalTask task = new SitePluginGoalTask();
    task.site = site;
    task.pluginGoal = pluginGoal;
    task.properties = properties;

    tasks.put(task.getId(), task);

    executorService.submit(task);
    LOGGER.info("Task {}: {}: Submitted", task.getId(), pluginGoal);
